...
The Page Society Template Generator follows a master-template architecture.

Every supported trim size owns one reusable template.

The template never changes except for the spine width.
...
must consume the geometry generated by the Master Template Engine.

No exporter is allowed to recreate geometry independently.